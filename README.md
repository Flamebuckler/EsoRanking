...
- **Live‑Search‑API:** ein Express‑Server (`server.js`) startet einen kleinen HTTP‑Service, der bei jeder Anfrage die entsprechenden Leaderboard‑Seiten rendert und sofort nach dem Accountnamen (oder Charakternamen) durchsucht. Die zu verarbeitenden Leaderboard‑URLs und die zu berücksichtigenden Regionen (pc-eu / pc-na) werden in `config.json` definiert. Bei der Umwandlung werden Account‑ und Charakternamen aus dem HTML getrennt (`<br>`-Trennung) und getrennt gespeichert. Außerdem entfernt der Parser `<strong>`‑ und `<small>`‑Tags, die auf der Seite zur Hervorhebung verwendet werden. Die Frontend‑Seite (`public/index.html` + `public/script.js`) ruft `/api/search?name=<account>` ab.
